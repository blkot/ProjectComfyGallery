...
| MEDIA-016–017 | Durable CG-owned MSS conversion, active-run reuse, restart recovery, and same-media publish observation | [Data model](architecture/data-model.md), [API](interfaces/api-and-integrations.md), [ADR-0013](decisions/0013-orchestrate-external-spatial-conversion.md) | MSS orchestration | `db.models.SpatialConversionRun`, `media.spatial_conversion`, spatial API/worker, `0013_spatial_conversion_runs`, `test_spatial_conversion.py` | Implemented; live MSS/NAS verification pending |
...
| WF-012–016 | Best-effort workflow input discovery, immediate capture, exact-byte deduplication, durable failures, and authenticated delivery | [Workflow intelligence](design/workflow-intelligence.md), [API](interfaces/api-and-integrations.md), [Data model](architecture/data-model.md), [ADR-0012](decisions/0012-capture-workflow-input-media.md) | Issue #12 | `workflow.input_media`, workflow-input API/worker, `0012_workflow_input_media`, `test_workflow_input_media.py` | Backend/API implemented; frontend presentation pending |
...
| LIB-020 | Media Detail starts and restores observable MSS conversion state, then refreshes the same media variant projection | [API](interfaces/api-and-integrations.md), [ADR-0013](decisions/0013-orchestrate-external-spatial-conversion.md) | MSS orchestration | `spatial-conversion-panel.tsx`, `spatial-conversion-panel.test.tsx`, frontend checks/build | Implemented; live MSS/NAS verification pending |
| LIB-021 | Context-preserving frontend Civitai preparation with local editable payload, evidence mapping, and disabled publishing | [Frontend UX](design/frontend-ux.md), [Product requirements](product/requirements.md) | post-rc.14 | `civitai-post.ts`, `civitai-post-page.tsx`, unit/page/detail-link tests, frontend checks/build | Frontend preparation implemented; backend publishing, connectivity, and proxy support deferred/not implemented |
...
| External conversion cannot replace media identity or succeed before CG observes its variant | MEDIA-016–017 | ADR-0013, spatial conversion service | `test_conversion_submits_original_and_observes_published_variant` |
...
| Captured workflow inputs remain available without ComfyUI | WF-012–016 | ADR-0012, workflow intelligence | Capture/dedup/API regression plus missing-input non-blocking case |
